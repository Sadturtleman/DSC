...
├── scoring/
│   ├── ScoringEngine          FSImage 분석 → pending_jobs(PENDING)
...
hdfs storagepolicies -satisfyStoragePolicy -path /tiering-test/sample.bin
...
psql -h localhost -U dsc -d dsc_tiering -c \
    "SELECT job_id, file_path, status, current_tier, target_tier
       FROM pending_jobs
      WHERE file_path = '/tiering-test/sample.bin'
