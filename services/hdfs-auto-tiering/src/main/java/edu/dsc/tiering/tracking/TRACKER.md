...
hdfs-auto-tiering 가 `pending_jobs` 테이블에 `DISPATCHED` 로 마킹한 job 을
...
# 외부 yaml 경로 지정
java -jar target/hdfs-auto-tiering.jar /etc/dsc/tracker.yaml
```
...
| `hdfs/HdfsApiCaller.java` | hdfs-auto-tiering | 정책 변경·SPS 호출 |
...
| `scheduler/BatchScheduler.java` | hdfs-auto-tiering | — |
...
```bash
# 1) 인프라 기동 (hdfs-auto-tiering 와 동일한 docker-compose 사용)
...
- **NN 부팅 race**: `HdfsPolicyChecker` 생성자가 즉시 NN RPC 를 시도함. NN 이 죽어있으면 Main 부팅 자체가 실패 → hdfs-auto-tiering 와 동일하게 lazy init / retry-with-backoff 개선 필요.
...
- [ ] **`recordHdfsFailure` 부분 성공 케이스 식별**: hdfs-auto-tiering 알려진 함정 — `setStoragePolicy` 는 됐지만 `satisfyStoragePolicy` 가 실패한 경우 tracker 가 식별해 감사 로그 보강
